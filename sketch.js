...
function getRiskTier(fraction) {
  if (fraction >= 0.99)
    return { label: "ALL  IN", color: "purple", bonusMult: 1.0 };
  else if (fraction >= 0.6)
    return { label: "RECKLESS", color: "red", bonusMult: 0.75 };
  else if (fraction >= 0.35)
    return { label: "BOLD", color: "amber", bonusMult: 0.45 };
  else if (fraction >= 0.15)
    return { label: "CALCULATED", color: "cyan", bonusMult: 0.2 };
...
function shieldPrice() {
  return max(10, floor(chips * 0.4));
}
function surgePrice() {
  return max(8, floor(chips * 0.28));
}
...
function draw() {
  ...
  revealFill +=
    ((revealHover && !revealBtn?.disabled ? 1 : 0) - revealFill) * 0.15;
...
function drawPowerupFlash() {
  ...
  let alpha, sc;
  if (pct > 0.65) {
    alpha = map(pct, 1.0, 0.65, 0, 255);
    sc = map(pct, 1.0, 0.65, 2.4, 1.0);
  } else if (pct > 0.2) {
    alpha = 255;
    sc = 1.0;
  } else {
    alpha = map(pct, 0.2, 0.0, 255, 0);
    sc = map(pct, 0.2, 0.0, 1.0, 0.75);
  }
  ...
  noStroke();
  rect(PAD, Y_ARENA, CW - PAD * 2, H_ARENA);
  ...
  fill(color(255, 255, 255, alpha * 0.1));
  noStroke();
...
function drawShopCard(x, y, w, h, item) {
  ...
  if (canAfford) {
    setShadow(`rgba(${item.glowRgb},0.45)`, 18);
  }
  let borderC = item.owned
    ? col("green")
    : canAfford
      ? col(item.borderKey)
      : col("muted", 100);
  ...
  if (item.owned) {
    ...
    if (canAfford) {
      fill(col("amber"));
      noStroke();
      rect(bx2 - 2, by2 - 2, bw2 + 4, bh2 + 4, 4);
      fill(color(120, 12, 12));
      noStroke();
      rect(bx2, by2 + 3, bw2, bh2 - 1, 2);
      let hov2 =
        mouseX >= bx2 &&
        mouseX <= bx2 + bw2 &&
        mouseY >= by2 &&
        mouseY <= by2 + bh2;
      fill(hov2 ? col("redHi") : col("red"));
      noStroke();
      rect(bx2, by2, bw2, bh2 - 3, 2);
      fill(col("white"));
      setFont(13, "display");
      textAlign(CENTER, CENTER);
      ...
      textAlign(CENTER, CENTER);
      text(
        chips < item.price ? "CAN'T AFFORD" : "SOLD OUT",
        bx2 + bw2 / 2,
        by2 + bh2 / 2,
      );
    }
...
function drawArena() {
  ...
  if (state === "BET") {
    ...
    text(
      act === 1
        ? "Place your bet, then reveal the number."
  ...
  if (state === "FLASH") {
    ...
    let fadeA = constrain(
      map(flashTimer, 0, flashDuration * 0.12, 0, 255),
      0,
      255,
    );
    ...
    fill(pct01 < 0.3 ? col("red") : col("amber"));
    noStroke();
...
function drawAct2Flash(ax, aw, fadeA) {
  ...
  for (let i = 0; i < sumNumbers.length; i++) {
    ...
    if (ft === 1 || ft === 3) scale(-1, 1);
    if (ft === 2 || ft === 3) scale(1, -1);
...
function drawRiskMeter(ax, aw) {
  ...
  fill(color(20, 15, 40));
  noStroke();
  rect(meterX, meterY, meterW, meterH, 7);

  // Tier zones (colored fills)
  let zones = [
    { from: 0.0, to: 0.15, col: "green" },
    { from: 0.15, to: 0.35, col: "cyan" },
    { from: 0.35, to: 0.6, col: "amber" },
    { from: 0.6, to: 0.99, col: "red" },
    { from: 0.99, to: 1.0, col: "purple" },
  ];
  for (let z of zones) {
  ...
  for (let ld of labelData) {
    ...
    let isActive =
      currentBet > 0 &&
      getRiskTier(currentBet / chips).label.startsWith(ld.label.split(" ")[0]);
  ...
  if (currentBet > 0) {
    ...
    textAlign(CENTER, TOP);
    text(
      "WIN: +" + potWin + " chips   LOSE: -" + currentBet + " chips",
      CW / 2,
      textY + 54,
    );
  } else {
...
function drawDoubleDownBtn(ax, aw) {
  ...
  fill(color(80, 10, 10));
  noStroke();
  rect(bx, by + 2, bw, bh, 2);
  fill(ddHover ? col("redHi") : col("red"));
  noStroke();
  rect(bx, by, bw, bh - 2, 2);
  clearShadow();
  fill(col("white"));
  setFont(11, "display");
  textAlign(CENTER, CENTER);
...
function drawBetSection() {
  ...
  if (state === "BET" && currentBet > 0)
    frac = constrain(currentBet / chips, 0, 1);
  let tier = getRiskTier(frac);
  let baseMult = getDiff().mult;
  let effMult =
    betForCalc > 0
  ...
  if (betForCalc > 0 || state !== "BET") {
    ...
    textAlign(RIGHT, TOP);
    fill(col("muted"));
    text("PAYOUT MULT", badgeX, badgeY);
...
function drawSplash() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    let hov =
      ...
      mouseX >= bx3 &&
      mouseX <= bx3 + bw3;
    fill(active ? col(d.color) : col("bord"));
    noStroke();
    rect(bx3 - 2, by3 - 2, bw3 + 4, bh3 + 4, 4);
    fill(active ? color(80, 60, 0) : color(80, 10, 10));
    noStroke();
    rect(bx3, by3 + 3, bw3, bh3 - 1, 2);
    let mf;
    if (active) {
      if (i === 0) mf = color(12, 139, 33);
      else if (i === 1) mf = color(200, 140, 0);
      else mf = color(176, 24, 24);
    } else mf = hov ? color(60, 20, 20) : color(30, 10, 10);
    fill(mf);
    noStroke();
    rect(bx3, by3, bw3, bh3 - 3, 2);
  ...
  fill(col("amber"));
  noStroke();
  rect(sbx - 3, sby - 3, sbw + 6, sbh + 6, 4);
  fill(color(120, 12, 12));
  noStroke();
  rect(sbx, sby + 4, sbw, sbh, 2);
  fill(hov ? col("redHi") : col("red"));
  noStroke();
  rect(sbx, sby, sbw, sbh - 4, 2);
  fill(col("white"));
  setFont(24, "display");
  textAlign(CENTER, CENTER);
...
function drawGameOver() {
  ...
  let chipColor =
    finalChips >= 100 ? "green" : finalChips >= 50 ? "amber" : "red";

  // ── Result card (compact, top half) ───────────────────────────────────────
  let cardW = CW - PAD * 2,
    cardH = 220,
    cardX = PAD,
    cardY = PAD + H_HEADER - 10;
  ...
  setShadow("rgba(210,35,45,0.5)", 18);
  fill(col("red"));
  text("GAME OVER!", midX, midY - 49);
  clearShadow();

  stroke(col("bord"));
  strokeWeight(1);
  line(cardX + 24, midY - 2, cardX + cardW - 24, midY - 2);
  noStroke();
  ...
  textAlign(LEFT, TOP);
  text("MODE", col2, lbY + 30);
  textAlign(LEFT, TOP);
  text("DATE", col3, lbY + 30);

  stroke(col("bord", 50));
  strokeWeight(1);
  line(cardX + 12, lbY + 42, cardX + cardW - 12, lbY + 42);
  noStroke();
  ...
  for (let i = 0; i < min(HS_MAX, max(highScores.length, 1)); i++) {
    ...
    if (entry) {
      let entryColor =
        entry.chips >= 100 ? "green" : entry.chips >= 50 ? "amber" : "red";
      fill(col(entryColor));
      setFont(13, "display");
      textAlign(LEFT, CENTER);
...
function mousePressed() {
  ...
  if (state === "ANSWER") {
    ...
    if (
      ddBtn &&
      inBtn(mouseX, mouseY, ddBtn) &&
      !doubledDown &&
      chipsBeforeBet >= lastBet &&
      lastBet > 0
    ) {
...
function startFlash() {
  ...
  let frac =
    chips + lastBet > 0 ? constrain(lastBet / (chips + lastBet), 0, 1) : 0;
...
function handleAnswer(val) {
  ...
  let correct =
    act === 1 ? val === correctAnswer : Math.abs(val - correctAnswer) === 0;

  if (correct) {
    let profit = floor(totalBet * effMult);
    chips += totalBet + profit;
    streak++;
    let tag =
      (riskMult > 1
        ? "  🔥×" + effMult.toFixed(1)
        : "  ×" + effMult.toFixed(1)) + (doubledDown ? " 2×BET" : "");
    setLog(
      "+" + profit + " chips! " + (act === 1 ? "Correct" : "EXACT") + "!" + tag,
      "good",
    );
    ...
    if (hasStreakShield) {
      ...
      setLog(
        "Shield saved you! +" +
          totalBet +
          " chips refunded. " +
          (act === 1 ? "It was " + correctAnswer : "Sum was " + correctAnswer) +
          ".",
        "info",
      );
    } else {
...
function handleTimeout() {
  ...
  if (hasStreakShield) {
    ...
    triggerPowerupFlash("🛡  STREAK SAVED!", "cyan");
    setLog(
      "TIME'S UP! Shield saved you! +" + totalBet + " chips refunded.",
      "info",
    );
  } else {
    streak = 0;
    setLog(
      "TIME'S UP!  -" + totalBet + " chips.  It was " + correctAnswer + ".",
      "bad",
    );
  }
  if (chips <= 0) {
    chips = 0;
    finalChips = 0;
    setLog("TIME'S UP! BUST — out of chips.", "bad");
  }
  state = "RESULT";
  resultTimer = 2000;
}
...
function nextRound() {
  if (chips <= 0) {
    finalChips = 0;
    saveHighScore(0, DIFF_SETTINGS[difficulty].label);
    state = "GAME_OVER";
    return;
  }
  ...
  if (actRound >= maxRounds) {
    if (act === 1) {
      ...
      finalChips = chips;
      saveHighScore(chips, DIFF_SETTINGS[difficulty].label);
      state = "GAME_OVER";
      return;
    }
  }
...
function generateChoicesAct1(correct, mirrored) {
  ...
  while (set.size < 4 && tries++ < 80) {
    let d,
      r = random();
    if (r < 0.4)
      d = correct + (random() > 0.5 ? 1 : -1) * floor(random(1, spread + 1));
